...
contract HSR {
    ...
    mapping(bytes32 => bool) songHashUsed;
    ...
    function updateSongPrice(uint256 _songID, uint256 _newPrice) public {
        require(identifyUser[msg.sender] == UserType.ARTIST, "Not an artist.");
        require(allSongs[_songID].artistAddress == msg.sender, "You are not the owner of this song.");
        require(_newPrice > 0, "Price must be greater than zero.");

        allSongs[_songID].price = _newPrice;
    }

    function removeSongFromSale(uint256 _songID) public {
        require(identifyUser[msg.sender] == UserType.ARTIST, "Not an artist.");
        require(allSongs[_songID].artistAddress == msg.sender, "You are not the owner of this song.");

        allSongs[_songID].price = 0;
    }
        ...
        )
    {
        ...
        return (
            ...
            s.artistName,
            s.genre,
            string(abi.encodePacked(s.hash)), // ✅ конвертируем bytes32 → string
            s.price,
